...
2. [LLM Requests](#llmreq)
    1. [Generate Assesment Questionnaire](#gaq)
       ...
       3. [User Change Determinants Dataset Example](#bddq)
...
#### Objective:
Here the aim is to construct a comprehensive questionnaire designed to populate the various datasets as articulated in the Data Model, which includes Users, Health Profile, Goal Measures, Goal Observations, and Behaviour Determinants. 
...
#### 3. [User Change Determinants Dataset](https://chat.openai.com/share/66e3dced-2129-4a03-aa57-f6b525aff8e5): <a name="bddq"></a>

### Persona 1:
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire11.jpg)
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire12.jpg)
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire13.jpg)
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire14.jpg)
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire15.jpg)



### Persona 2:
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire21.jpg)
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire22.jpg)
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire23.jpg)
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire24.jpg)
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire25.jpg)
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/BD_questionnaire26.jpg)
...
+ Filling User Change Determinants dataset for a particular goal  
+ Filling User Profile dataset 
+ Filling Goal Measures dataset 
+ Filling Goal Observations dataset

### Examples

### 1. [User Change Determinants dataset](https://chat.openai.com/share/0e721d92-e6e5-49b5-94b0-d8db18cb9060) <a name="bdd"></a>

###### Persona 1:
![task3_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/determinant_structure11.jpg)
![task3_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/determinant_structure12.jpg)



###### Persona 2:
![task3_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/determinant_structure21.jpg)
![task3_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_v3/determinant_structure22.jpg)
...
#### Objective:
Once all relevant datasets, namely User Profile, Goal, Goal Measures, Goal Observations, User Change Determinants, are comprehensively populated, the next step involves creating a behavioral change plan to facilitate the achievement of set goals. For our purposes, we are adopting a standard timeline of one week for generating this behavioral change plan. The plan generation commences with the identification of appropriate behavioral change techniques(BCT), leveraging the CALO-RE Taxonomy for this purpose. There are Two primary tasks assigned to the LLM in this plan generation phase. The first task is the selection of the three most suitable behavioral change techniques from the 40 identified in the CALO-RE Taxonomy] (https://www.dhi.ac.uk/san/waysofbeing/data/health-jones-michie-2011a.pdf), this selection is primarily based on the individual's stage of behavioral change. This approach is taken in light of the emphasis placed on tailoring behavioral change techniques according to the stage of behavioral change in [this paper] (https://www.researchgate.net/publication/349007558_Tailoring_Persuasive_and_Behaviour_Change_Systems_Based_on_Stages_of_Change_and_Motivation). The second task involves generating a behavioral change plan, structured around the identified techniques, based on context information to make the plan personalised.

#### Context:
The context involves the dataset entries pertaining to the user. These entries, drawn from the datasets User Profile, Goal, Goal Measures, Goal Observations, User Change Determinants will be used to generate the necessary text information.
...
#### Objective:
Once the plan is formulated and each component of the plan is recorded in the Plan_Entry dataset, it becomes necessary to generate specific intervention text and feedback retreival text at the corresponding execution time of each plan entry. This is dependent on the activity the user is expected to perform, the behavior change technique applied, and the form of intervention. To this end, Plan Entries, identified by their Entry_Id, along with other relevant user information are provided to the LLM as context. The LLM will then generate suitable intervention text associated with each Entry_Id.

#### Context:
The context encompasses dataset entries related to the user. The relevant information, drawn from the datasets Users, Health_Profile, User_Goal, Goal_Observations, User Change Determinants and Plan_Entry, will be analyzed to generate the necessary intervention text.
...
#### Context:
The context encompasses user-related information sourced from the datasets Users, Health_Profile, User_Goal, Goal_Observations, User Change Determinants. Additionally, the current plan and feedback text are also included in the context.
...
+ Add information in User Change Determinants dataset based on User_Goal dataset
+ User goal assesment
...
##### User Change Determinants Dataset:
![task1_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_LLM/task1_User Change Determinants_1.jpg)
![task1_User Change Determinants_2](https://github.com/mhb3194/HICSS_Data/blob/main/images_LLM/task1_User Change Determinants_2.jpg) 
...
+ **Stage of Change Identification** : Identifying current value of user's  behaviour determinant Stage of Change (precontemplation/contemplation/preperation/action/maintainance) based on Users, Health_Profile, User_Goal, Goal_Observations, User Change Determinants datasets information (*[Context]: Users dataset, Health_Profile dataset, User_Goal dataset, Goal_Observations dataset, User Change Determinants dataset ; [Output Format]: class name*)
...
#### Application Scenarios
Based on answers of the questionnaires
+ Filling User Change Determinants dataset for a particular goal  (*[Context]: User Change Determinants dataset, User_Response ; [Output Format]: table*)
...
![task3_User Change Determinants_1](https://github.com/mhb3194/HICSS_Data/blob/main/images_LLM/task3_User Change Determinants_1.jpg)
![task3_User Change Determinants_2](https://github.com/mhb3194/HICSS_Data/blob/main/images_LLM/task3_User Change Determinants_2.jpg)
...
#### Context:
The context involves the dataset entries pertaining to the user. These entries, drawn from the datasets Users, Health_Profile, User_Goal, Goal_Observations, and User Change Determinants, will be used to generate the necessary text information.
...
*Weight Management and Physicaltask1_User Change Determinants_1 Activity: "Adopt a balanced diet and increase physical activity to manage BMI and enhance overall health."*
...
![determinant_query](https://github.com/mhb3194/HICSS_Data/blob/main/LLM_Tasks/User Change Determinants_1_new_p1.jpg) 

#### Answer

![determinant_ans_1](https://github.com/mhb3194/HICSS_Data/blob/main/LLM_Tasks/User Change Determinants_1_new_p2.jpg) 
![determinant_ans_2](https://github.com/mhb3194/HICSS_Data/blob/main/LLM_Tasks/User Change Determinants_1_new_p3.jpg) 
...
![determinant_extraction_query](https://github.com/mhb3194/HICSS_Data/blob/main/LLM_Tasks/User Change Determinants_2_new.jpg)


#### Answer

![determinant_extraction_answer](https://github.com/mhb3194/HICSS_Data/blob/main/LLM_Tasks/User Change Determinants_3_new.jpg)
